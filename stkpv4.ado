...
prog def stkpv4
	...
	gen col = 2
	qui{
		replace col = open[_n] > close[_n-1] if _n != 1
		drop if col == 2
		sum volume
	}
	local max = `r(max)'
...
prog def graphcandelsticks
	...
	yla(#4) yti("股价(元)") name(hilo, replace) xscale(off) leg(order(`order') pos(3) ring(0)) nodraw sch(plotplain) || `cmd'
	tw ///
